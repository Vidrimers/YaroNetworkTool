# Руководство по развертыванию VPN Management System

## Архитектура системы

```
VPN Сервер (144.124.237.222)
├── ~/xray-vpn/                 # Основной репозиторий
│   ├── api/server.js           # Management API (порт 333)
│   ├── database/vpn.db         # База данных SQLite
│   ├── configs/                # Конфигурации X-Ray
│   └── scripts/                # CLI скрипты
│
├── ~/yaronetworktool/          # Telegram бот
│   └── bot/yaronetworktool_bot.js
│
└── /usr/local/etc/xray/        # X-Ray Core
    └── config.json
```

## Часть 1: Первоначальная установка на сервере

### Шаг 1: Подключение к серверу

```bash
ssh root@144.124.237.222
```

### Шаг 2: Установка необходимых компонентов

```bash
# Обновить систему
apt update && apt upgrade -y

# Установить Node.js 18+
curl -fsSL https://deb.nodesource.com/setup_18.x | bash -
apt install -y nodejs

# Установить PM2 глобально
npm install -g pm2

# Установить Git (если не установлен)
apt install -y git

# Установить SQLite3
apt install -y sqlite3

# Проверить версии
node --version    # должно быть >= 18.0.0
npm --version
pm2 --version
git --version
```

### Шаг 3: Клонирование основного репозитория

```bash
# Перейти в домашнюю директорию
cd ~

# Клонировать репозиторий (замените URL на ваш)
git clone https://github.com/YOUR_USERNAME/xray-vpn-server.git xray-vpn

# Перейти в директорию
cd xray-vpn

# Установить зависимости
npm install --production
```

### Шаг 4: Настройка .env для API

```bash
# Создать .env файл
cat > .env << 'EOF'
# API Configuration
API_PORT=3000
NODE_ENV=production

# Database
DB_PATH=~/xray-vpn/database/vpn.db

# JWT Secret (сгенерируйте случайную строку)
JWT_SECRET=your-super-secret-jwt-key-change-this

# Server
SERVER_IP=144.124.237.222
EOF

# Установить права
chmod 600 .env
```

### Шаг 5: Инициализация базы данных

```bash
# Создать директорию для базы данных
mkdir -p ~/xray-vpn/database

# Инициализировать базу данных
sqlite3 ~/xray-vpn/database/vpn.db < database/init.sql

# Проверить что таблицы созданы
sqlite3 ~/xray-vpn/database/vpn.db "SELECT name FROM sqlite_master WHERE type='table';"

# Должны появиться: clients, extension_requests, traffic_logs
```

### Шаг 6: Запуск API через PM2

```bash
cd ~/xray-vpn

# Запустить API
pm2 start api/server.js --name vpn-api --time

# Проверить статус
pm2 status

# Просмотреть логи
pm2 logs vpn-api --lines 50

# Сохранить конфигурацию PM2 для автозапуска
pm2 save
pm2 startup
# Выполните команду которую выведет pm2 startup
```

### Шаг 7: Проверка работы API

```bash
# Проверить health endpoint
curl http://localhost:3000/health

# Должен вернуть: {"status":"ok","timestamp":"..."}

# Проверить список клиентов (должен быть пустой)
curl http://localhost:3000/api/v1/clients

# Должен вернуть: {"success":true,"count":0,"clients":[]}
```

## Часть 2: Установка Telegram бота

### Шаг 1: Клонирование репозитория бота

```bash
cd ~

# Клонировать репозиторий (замените URL на ваш)
git clone https://github.com/YOUR_USERNAME/yaronetworktool.git yaronetworktool

# Перейти в директорию
cd yaronetworktool

# Установить зависимости
npm install --production
```

### Шаг 2: Настройка .env для бота

```bash
# Создать .env файл
cat > .env << 'EOF'
# Telegram Bot Configuration
TELEGRAM_BOT_TOKEN=8203314241:AAG53unoLQMWfLMYxWNYdPMeG1TGeWCS50Q
TELEGRAM_ADMIN_ID=137981675

# API Configuration (локальный API на том же сервере)
API_BASE_URL=http://localhost:333
SERVER_IP=144.124.237.222

# Database (используем ту же базу что и API)
DB_PATH=~/xray-vpn/database/vpn.db

# SSH Configuration (не нужно если на том же сервере)
# SSH_HOST=localhost
# SSH_PORT=22
# SSH_USER=root
# SSH_PASSWORD=your-password
EOF

# Установить права
chmod 600 .env
```

### Шаг 3: Запуск бота через PM2

```bash
cd ~/yaronetworktool

# Запустить бота
pm2 start bot/yaronetworktool_bot.js --name vpn-bot --time

# Проверить статус
pm2 status

# Просмотреть логи
pm2 logs vpn-bot --lines 50

# Сохранить конфигурацию
pm2 save
```

### Шаг 4: Проверка работы бота

```bash
# Проверить что бот запущен
pm2 list

# Должны быть два процесса:
# - vpn-api (online)
# - vpn-bot (online)

# Проверить логи бота
pm2 logs vpn-bot --lines 20

# Должно быть: "[YaroNetworkTool VPN Bot] Запущен"
```

Теперь отправьте `/start` боту в Telegram - должно появиться меню!

## Часть 3: Настройка автоматического обновления через Git

### Подготовка скриптов обновления

```bash
# Сделать скрипты исполняемыми
chmod +x ~/xray-vpn/scripts/kvn.sh
chmod +x ~/xray-vpn/scripts/kvn-stop.sh
chmod +x ~/yaronetworktool/bot/kvn-bot.sh
chmod +x ~/yaronetworktool/bot/kvn-bot-stop.sh
```

### Использование скриптов обновления

**Обновление API (с проверками и бэкапами):**
```bash
cd ~/xray-vpn
./scripts/kvn.sh
```

**Что делает kvn.sh:**
1. ✅ Создает бэкап базы данных
2. ✅ Проверяет наличие обновлений
3. ✅ Получает обновления из Git (git pull)
4. ✅ Обновляет зависимости если изменился package.json
5. ✅ Перезапускает через PM2
6. ✅ Проверяет health endpoint
7. ✅ Автоматически откатывает при ошибке
8. ✅ Очищает старые бэкапы

**Обновление бота (быстрое):**
```bash
cd ~/yaronetworktool
./bot/kvn-bot.sh
```

**Остановка сервисов:**
```bash
cd ~/xray-vpn
./scripts/kvn-stop.sh        # Остановить API

cd ~/yaronetworktool
./bot/kvn-bot-stop.sh        # Остановить бота
```

**Что делает kvn-bot.sh:**
1. ✅ Получает обновления из Git (git pull)
2. ✅ Перезапускает через PM2
3. ✅ Показывает статус

**Примечание:** В kvn-bot.sh обновление зависимостей (npm install) закомментировано. Раскомментируйте если изменился package.json.

## Часть 4: Полезные команды PM2

### Управление процессами

```bash
# Список всех процессов
pm2 list

# Статус конкретного процесса
pm2 show vpn-api
pm2 show vpn-bot

# Перезапуск
pm2 restart vpn-api
pm2 restart vpn-bot
pm2 restart all

# Остановка
pm2 stop vpn-api
pm2 stop vpn-bot

# Запуск
pm2 start vpn-api
pm2 start vpn-bot

# Удаление из PM2
pm2 delete vpn-api
pm2 delete vpn-bot
```

### Просмотр логов

```bash
# Логи в реальном времени
pm2 logs vpn-api
pm2 logs vpn-bot
pm2 logs all

# Последние N строк
pm2 logs vpn-api --lines 100

# Очистить логи
pm2 flush

# Логи развертывания
tail -f /var/log/kvn-deployment.log
tail -f /var/log/kvn-bot-deployment.log
```

### Мониторинг

```bash
# Интерактивный мониторинг
pm2 monit

# Статистика использования ресурсов
pm2 status
```

## Часть 5: Тестирование системы

### Тест 1: Проверка API

```bash
# Health check
curl http://localhost:3000/health

# Создать тестового клиента
curl -X POST http://localhost:3000/api/v1/clients \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Test User",
    "telegram_id": 123456789,
    "subscription_days": 30,
    "traffic_limit_gb": 100
  }'

# Получить список клиентов
curl http://localhost:3000/api/v1/clients
```

### Тест 2: Проверка бота

1. Отправьте `/start` боту в Telegram
2. Должно появиться админ меню (если ваш ID = TELEGRAM_ADMIN_ID)
3. Попробуйте команду `/list_clients`
4. Должен появиться созданный тестовый клиент

### Тест 3: Проверка запросов на продление

1. Создайте клиента с вашим Telegram ID через `/add_client`
2. От имени клиента нажмите `🔑 Запросить ключ`
3. Выберите период (например, 3 месяца)
4. Администратор должен получить уведомление
5. Нажмите `✅ Разрешить`
6. Клиент должен получить уведомление об одобрении

## Часть 6: Workflow разработки и обновления

### Локальная разработка

```bash
# На вашем компьютере
git clone <repo-url>
cd xray-vpn-server

# Внести изменения
# ... редактирование файлов ...

# Закоммитить
git add .
git commit -m "Добавлена новая функция"

# Отправить на GitHub
git push origin main
```

### Обновление на сервере

```bash
# На сервере
ssh root@144.124.237.222

# Обновить API
cd /opt/xray-vpn
./scripts/kvn.sh

# Обновить бота
cd /opt/yaronetworktool
./bot/kvn-bot.sh
```

**Скрипты автоматически:**
- Получат обновления из Git
- Перезапустят приложения через PM2
- Покажут статус

## Часть 7: Troubleshooting

### API не запускается

```bash
# Проверить логи
pm2 logs vpn-api --lines 100

# Проверить порт
netstat -tulpn | grep 3000

# Проверить .env
cat /opt/xray-vpn/.env

# Проверить базу данных
sqlite3 /opt/xray-vpn/database/vpn.db ".tables"

# Перезапустить
pm2 restart vpn-api
```

### Бот не отвечает

```bash
# Проверить логи
pm2 logs vpn-bot --lines 100

# Проверить токен
cat /opt/yaronetworktool/.env | grep TELEGRAM_BOT_TOKEN

# Проверить подключение к API
curl http://localhost:3000/health

# Перезапустить
pm2 restart vpn-bot
```

### Ошибка при обновлении

```bash
# Вручную откатить Git
cd ~/xray-vpn
git reset --hard HEAD@{1}
pm2 restart vpn-api

# Для бота
cd ~/yaronetworktool
git reset --hard HEAD@{1}
pm2 restart vpn-bot
```

### База данных повреждена

```bash
# Восстановить из бэкапа
cd ~/xray-vpn-backup
ls -lt vpn_*.db | head -1  # найти последний бэкап

# Восстановить
cp vpn_20260126_123456.db ~/xray-vpn/database/vpn.db

# Перезапустить
pm2 restart all
```

## Часть 8: Безопасность

### Настройка файрвола

```bash
# Разрешить только необходимые порты
ufw allow 22/tcp      # SSH
ufw allow 443/tcp     # X-Ray (HTTPS)
ufw allow 333/tcp     # Management API
ufw enable

# Бот подключается к API через порт 333
```

### Защита .env файлов

```bash
# Установить правильные права
chmod 600 ~/xray-vpn/.env
chmod 600 ~/yaronetworktool/.env

# Проверить
ls -la ~/xray-vpn/.env
# Должно быть: -rw------- (только владелец может читать)
```

### Регулярные бэкапы

```bash
# Создать директорию для бэкапов
mkdir -p ~/xray-vpn-backup

# Создать cron задачу для бэкапа базы данных
crontab -e

# Добавить строку (бэкап каждый день в 3:00)
0 3 * * * cp ~/xray-vpn/database/vpn.db ~/xray-vpn-backup/vpn_daily_$(date +\%Y\%m\%d).db
```

## Резюме

✅ **API запущен:** http://localhost:333 (PM2: vpn-api)
✅ **Бот запущен:** PM2: vpn-bot
✅ **Автообновление:** ./scripts/kvn.sh и ./bot/kvn-bot.sh
✅ **Автозапуск:** PM2 startup настроен
✅ **Бэкапы:** Настройте cron для автоматического бэкапа БД

**Основные команды:**
```bash
pm2 status                    # Статус всех процессов
pm2 logs                      # Логи всех процессов
cd ~/xray-vpn && ./scripts/kvn.sh              # Обновить API
cd ~/yaronetworktool && ./bot/kvn-bot.sh       # Обновить бота
cd ~/xray-vpn && ./scripts/kvn-stop.sh         # Остановить API
cd ~/yaronetworktool && ./bot/kvn-bot-stop.sh  # Остановить бота
```

Система готова к работе! 🚀
