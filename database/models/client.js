...
class ClientModel {
  constructor(dbPath) {
    this.db = new sqlite3.Database(dbPath, (err) => {
      ...
      // Включаем WAL mode для конкурентного доступа
      this.db.run('PRAGMA journal_mode=WAL', (err) => {
        if (err) console.error('Ошибка включения WAL:', err.message);
      });
      // Устанавливаем busy timeout 5 секунд
  ...
  async extendSubscription(uuid, days) {
    ...
    try {
      ...
      console.error(`[extendSubscription] ❌ Ошибка синхронизации с Xray для ${client.name} (${uuid}):`, error.message);
      console.error(`[extendSubscription] Stack trace:`, error.stack);
